...
function add_tb_list_go(name){
		...
		if(val.length>0){	
		    data={mode:'admin_edit_db_list',tablename:name,title:val,keywords:job};
//console.log(data)
		    bak=get_ajax('/API',data); 
...
function edit_tlebe_data_mod(name,data,pnumber,val,p){
		...
		$.each(datas, function (n) {
			//if(n!='ID'){//<textarea rows="10" cols="120" id="'+n+'">'+Base64.decode(datas[n])+'</textarea>}
		        if(n=='page'){
                                      vte='<div id="'+n+'">'+Base64.decode(datas[n])+'</div>'
                        }else{
                                      if(n=='time'){
                                            datas['time']=tostamp(datas['time'])

                                      }                                           
                                      vte='<input type="text" id="'+n+'" value="'+datas[n]+'">'
...
function save_data(name,data){	
		...
		$.each(datas, function (n) {
			...
			if(n!='page'){
                ...
                Tdata={'mode':'edit_db','tablename':name,'title':datas.ID,'keywords':job}
...
function add_input_go() {
	...
	if (db_name.length > 0 & val.length >0){
		         ...
		         $('#db').html(db_name+' '+val)
                         data={'mode':'admin_add_db_table','tablename':db_name,'keywords':val}
                         bak=get_ajax('/API',data);
...
function del_db_tble(p) {//console.log(p)
	  if(confirm("确定?")){
		  ...
		  if (str.indexOf(p) != -1){
			...
			data={mode:'admin_del_db_table',tablename:p}
                        bak=get_ajax('/API',data);
                        if(bak.info=='y'){database();}
		  }	  
	  }
